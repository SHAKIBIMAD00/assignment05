      ...
      <div class="card-body">
        <div class="card-top">
          <div class="status-icon">
            <img src="${item.status === "open" ? openIcon : closedIcon}">
          </div>
...
function getFilteredIssues() {
  return issues.filter(issue => {
    const statusMatch =
      activeStatus === "all" ? true : issue.status === activeStatus;
...
function openPopup(item) {
  ...
  modalStatus.className =
    "status-btn-modal " + (item.status === "open" ? "status-open" : "status-closed");

  document.getElementById("modal-author-info").innerText =
    `Opened by ${item.author} • ${item.date}`;
